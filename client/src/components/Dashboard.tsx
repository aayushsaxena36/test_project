...
const ROW_HEIGHT = 280; // adjust if needed

// Dashboard component for monitoring patient vitals
export const Dashboard = () => {
    const patientsMapRef = useRef<Map<string, PatientVitals>>(new Map());
    const [patients, setPatients] = useState<PatientVitals[]>([]);
    const [isConnected, setIsConnected] = useState(false);

    const PatientRow = memo(({ patient, style }: any) => {
        return (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                        ...
                        <div className="bg-gray-50 p-3 rounded flex items-center justify-center">
                            <div className="text-center">
                                ...
                                <div
                                    className={`text-sm font-semibold px-3 py-1 rounded-full ${patient.oxygenSaturation < 95 || patient.heartRate > 100 || patient.heartRate < 60
                                        ? 'bg-red-100 text-red-800'
                                        : 'bg-green-100 text-green-800'
                                        }`}
                                >
                                    {patient.oxygenSaturation < 95 || patient.heartRate > 100 || patient.heartRate < 60
                                        ? 'CRITICAL'
                                        : 'STABLE'}
                                </div>
                            </div>
    ...
    useEffect(() => {
        ...
        socket.on("vitals_update", (updates: PatientVitals[]) => {
            ...
            updates.forEach(update => {
                const prev = patientsMapRef.current.get(update.id);

                // Shallow compare to avoid useless updates
                if (!prev || prev.timestamp !== update.timestamp) {
                    patientsMapRef.current.set(update.id, update);
                    changed = true;
                }
            });
